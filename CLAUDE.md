...
Real-estate lead-gen landing page for Noble Crystal Riverside (Đường Đào Trí, Phường Phú Thuận, Quận 7, HCMC — developed by Sunshine Group), plus an internal admin module. Status as of the latest commit:
...
- **Entry popup** (`EntryPopup.tsx`): Radix `Dialog` that auto-opens 3s after mount, gated by `sessionStorage["hasSeenEntryPopup"]` so it only shows once per browser session; embeds the same `LeadForm` (`source="Popup Chào Mừng"`).
...
- **Cloned from the `sunshine-sky-city` boilerplate (via the `chauthanhvuong` and `hovanchuong` clones); mounted at `www.minhhoanghome.site/noble-crystal-riverside`** via Next.js `basePath` on its own standalone domain — no gateway/rewrite project involved (unlike the base boilerplate). See "Domain architecture" below — this is the one change most likely to break silently if a future edit adds a raw local image `src` or a bare internal `fetch()` call.
- **White-label boilerplate refactor applied**: `src/config/site.ts` centralizes `layoutOrder` (section render order, consumed by `src/app/page.tsx`'s `sectionComponents` registry) and `images`/`sections` (every local image path and array-based section data — amenities, floor plans, apartment types, tower floor plates — grouped by the section that consumes it). A future clone only ever needs to touch domain/phone/`layoutOrder`/`images`/`sections` in that one file plus the files under `public/images/`, never the `.tsx` components. `scripts/clone-project.mjs` (inherited from the base boilerplate) automates copying the repo to a new sibling project.
...
## Domain architecture: www.minhhoanghome.site (standalone domain + basePath, no gateway)

Unlike the base `sunshine-sky-city` boilerplate this lineage started from, this project does **not** run behind the `canhoquan7-gateway` rewrite project. It owns its own domain, `www.minhhoanghome.site`, assigned directly to this app's own Vercel project (`minhhoanghome/hoangminhhome`). There is no separate gateway repo involved and nothing to add there.

The site lives at `/noble-crystal-riverside` rather than the domain root (`next.config.mjs` sets `basePath: "/noble-crystal-riverside"`), purely as a URL-structure choice for this project, not for multi-tenant routing. Consequences of `basePath`, all already applied — same mechanics as the base boilerplate, just a different domain, same prefix as before:
...
- `siteConfig.url` is the full external URL (`https://www.minhhoanghome.site/noble-crystal-riverside`, no trailing slash) used for canonical/OpenGraph. `siteConfig.seo.ogImage` is a **full absolute URL**, not a root-relative path — combining a leading `/` with `metadataBase` resolves against the domain root and silently drops the `/noble-crystal-riverside` segment. `metadataBase` itself is constructed as `` new URL(`${siteConfig.url}/`) `` (trailing slash added) so any *future* relative metadata field resolves correctly instead of dropping the last path segment.

**Deployed**: the Vercel project points directly at the `www.minhhoanghome.site` domain (Project → Settings → Domains) — no rewrites, no second repo. A request to the bare domain root (`www.minhhoanghome.site/`, no `/noble-crystal-riverside`) 404s unless a redirect is added separately; that is expected with this setup, not a bug. Before the custom domain's DNS propagates, the project is reachable at its default `hoangminhhome.vercel.app` (root basePath, no prefix) — that is the fallback URL to use for testing until DNS/domain assignment completes.

**GitHub / Vercel / Supabase**: this project has its own GitHub repo (`lehoainam31011991-cmyk/hoangminhhome`), its own Vercel project (`vercel.com/minhhoanghome/hoangminhhome`), and its own Supabase project (`ywlcinjtdrfxtvmbijrq`) — separate from the base boilerplate's and the earlier `chauthanhvuong`/`hovanchuong` clones this lineage passed through. Never reuse credentials across these projects.
